...
int main()
{
	try
	{
		// curl --verbose --cacert server.crt --location --request GET
		...
		router_middleware->add_router(
									  std::make_shared<http::route>(http::http_method::get, "/user/:username"),
									  [](http::http_request const &request, http::http_response &response) {
										  response.set_body("OLAAA");
									  });
