...
bool session::disconnect_async(bool dispatch)
{
    ...
    auto disconnect_handler = [this]() -> void
    {
        ...
        {
            logger_->error(
                "[SslSession][disconnect_async][disconnect_handler] Error on close socket. Code: {}. Message: {}",
                ec.value(),
                ec.message());
            return;
        }
...
size_t session::send(void const* buffer, size_t size)
{
    ...
    size_t sent = asio::write(stream_socket_, asio::buffer(buffer, size), ec);
    if (sent <= 0)
    {
        send_error(ec);
        disconnect(ec);
...
session::try_send()
{
    if (sending_)
    {
        logger_->error("[SslSession][send_try_send] It already sending...");
        return;
    }
    ...
    stream_socket_.async_write_some(
